...
var resizePizzas = function(size) {
  ...
  // Change querySelector to getElementById and store it in a variable outside the function.
  ...
  /* Edit: Replaces querySelector to getElementsByClassName to improve performance and put in a variable pizzaElements.
  Variable is stored outside the function */
  var pizzaElements = document.getElementsByClassName("randomPizzaContainer");

  function changePizzaSizes(size) {
    /* Inserting determineDX function here. changePizzaSizes() iterated through all pizza elements which are all the same size. This is redundant and is optimized */
    // oldWidth only takes first index value since all sizes are the same
    var oldWidth = pizzaElements[0].offsetWidth;
    ...
    for (var i = 0; i < pizzaElements.length; i++) {
      // dx and newwidth no longer needed here since we've already computed them
...
function updatePositions() {
  ...
  // Change querySelectorAll to getElementsByClassName for faster perf.
  var items = document.getElementsByClassName('mover');
  //Moved out of loop so variable is not always created
  var phase;
  //Moved out of loop since it's a constant and doesn't always need to be recalculated
